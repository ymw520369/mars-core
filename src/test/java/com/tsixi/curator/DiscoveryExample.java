...
public class DiscoveryExample {
	...
	public static void main(String[] args) throws Exception {
		...
		Map<String, ServiceProvider<InstanceDetails>> providers = Maps
				.newHashMap();
		try {
			...
			serviceDiscovery = ServiceDiscoveryBuilder
					.builder(InstanceDetails.class).client(client)
		...
		try {
			...
			while (!done) {
				...
				if (operation.equalsIgnoreCase("help")
					...
					listRandomInstance(args, serviceDiscovery, providers,
							command);
	...
	private static void listRandomInstance(String[] args,
			ServiceDiscovery<InstanceDetails> serviceDiscovery,
			Map<String, ServiceProvider<InstanceDetails>> providers,
			String command) throws Exception {
		// this shows how to use a ServiceProvider
		// in a real application you'd create the ServiceProvider early for the
		// dao(s) you're interested in

		if (args.length != 1) {
			System.err.println("syntax error (expected random <name>): "
					+ command);
			return;
		}
	...
	private static void listInstances(
			ServiceDiscovery<InstanceDetails> serviceDiscovery)
			throws Exception {
		// This shows how to query all the instances in dao discovery
	...
	private static void deleteInstance(String[] args, String command,
		...
		if (args.length != 1) {
			System.err.println("syntax error (expected delete <name>): "
					+ command);
			return;
		}

		final String serviceName = args[0];
		ExampleServer server = Iterables.find(servers,
				new Predicate<ExampleServer>() {
					@Override
					public boolean apply(ExampleServer server) {
						return server.getThisInstance().getName()
	...
	private static void addInstance(String[] args, CuratorFramework client,
			String command, List<ExampleServer> servers) throws Exception {
		// simulate a new instance coming up
		// in a real application, this would be a separate process

		if (args.length < 2) {
			System.err
					.println("syntax error (expected add <name> <description>): "
							+ command);
			return;
		}
		...
		String serviceName = args[0];
		ExampleServer server = new ExampleServer(client, PATH, serviceName,
	...
	private static void printHelp() {
		...
		System.out
				.println("add <name> <description>: Adds a mock dao with the given name and description");
		System.out
		...
		System.out
				.println("random <name>: Lists a random instance of the dao with the given name");
