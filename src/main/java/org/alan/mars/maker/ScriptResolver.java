...
import org.alan.mars.maker.annotation.BufferMapping;
...
public class ScriptResolver implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public CtClass createClass(ClassPool pool, ScriptInfo scriptInfo) {
        try {
            ...
            CtField field = CtField.make("private " + scriptInfo.getScriptClass().getName() + " data = null;", clazz);
            ...
            method = CtMethod.make("public void setBean(Object b){" +
                    "this.data = (" + scriptInfo.getScriptClass().getName() + ")b;" +
                    "}", clazz);
            clazz.addMethod(method);
            return clazz;
        } catch (Exception e) {
